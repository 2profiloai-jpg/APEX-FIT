...
function guessCategory(line) {
    ...
    if (line.includes('squat') || line.includes('stacco') || line.includes('affond') || line.includes('leg') || line.includes('glute') || line.includes('calf') || line.includes('hip') || line.includes('step') || line.includes('sissy')) return 'Gambe';
    if (line.includes('panca') || line.includes('press') || line.includes('croci') || line.includes('push') || line.includes('chest') || line.includes('pec deck') || line.includes('dip') || line.includes('spinte')) return 'Petto';
    if (line.includes('rematore') || line.includes('pull') || line.includes('chin') || line.includes('lat ') || line.includes('row') || line.includes('trazion') || line.includes('scrollate') || line.includes('shrug')) return 'Schiena';
    if (line.includes('alzate') || line.includes('spalle') || line.includes('shoulder') || line.includes('fly') || line.includes('face pull')) return 'Spalle';
    ...
    if (line.includes('crunch') || line.includes('plank') || line.includes('twist') || line.includes('sit-up') || line.includes('core') || line.includes('ab ') || line.includes('bug') || line.includes('flutter') || line.includes('v-up') || line.includes('bird') || line.includes('superman')) return 'Core';
...
lines.forEach(line => {
    if (line.match(/^[A-Z0-9.\s'()]+$/) && line.length < 30 && !line.includes('Squat') && !line.includes('Press') && !line.includes('Curl') && !line.includes('Row') && !line.includes('Pull') && !line.includes('Push')) {
        ...
        // Let's guess the real category!
        let cat = guessCategory(line);
        if (cat === 'Altro') cat = 'Petto'; // default safeguard
        
        let equipment = 'Multifunzione';
        if (line.toLowerCase().includes('manubri') || line.toLowerCase().includes('dumbell')) equipment = 'Manubri';
        else if (line.toLowerCase().includes('bilanciere') || line.toLowerCase().includes('trap bar') || line.toLowerCase().includes('bar')) equipment = 'Bilanciere';
        else if (line.toLowerCase().includes('cavi') || line.toLowerCase().includes('cavo') || line.toLowerCase().includes('corda')) equipment = 'Cavi';
        else if (line.toLowerCase().includes('machine') || line.toLowerCase().includes('smith') || line.toLowerCase().includes('multipower')) equipment = 'Macchinario';
        else if (line.toLowerCase().includes('corpo libero') || line.toLowerCase().includes('push-up') || line.toLowerCase().includes('plank') || line.toLowerCase().includes('trazion') || line.toLowerCase().includes('dips')) equipment = 'Corpo Libero';
        else if (line.toLowerCase().includes('kettlebell')) equipment = 'Kettlebell';
...
fs.writeFileSync('src/new_exercises.ts', output.join(',\\n'));
